class Organization {
  ...
  insertNewStaff(org_id, uid) {
    ...
    var promise = new Promise((resolve, reject) => {
      organization
        .update({ _id: org_id }, { $addToSet: { staff: { _id: uid } } })
        .then(data => {
          if (data.nModified > 0)
            resolve({ success: "staff added successfuly" });
          else reject({ error: "staff already added" });
        })
        .catch(err => {
          reject({ error: err });
        });
  ...
  insertNewMember(org_id, uid) {
    ...
    var promise = new Promise((resolve, reject) => {
      organization
        .update({ _id: org_id }, { $addToSet: { member: { _id: uid } } })
        .then(data => {
          if (data.nModified > 0)
            resolve({ success: "member added successfuly" });
          else reject({ error: "member already added" });
        })
        .catch(err => {
          reject({ error: err });
        });
